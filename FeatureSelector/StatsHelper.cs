...
namespace FeatureSelector
{
    public class StatsHelper
    {
        public Dictionary<string, Dictionary<string, int>> GetFrequencies(Sentence[] documents)
        {
            ...
            {
                for (int j = 0; j < documents[i].Words.Length; j++)
                {
                    //bag of words
                    AddTermFrequency(featureFrequencies, documents[i].Words[j], i);                   
                }
            }
        ...
        public Dictionary<string, Stats> GetFeaturesStats(HashSet<string> wordFeatures, Dictionary<string, Dictionary<string, int>> termFrequencies, string classification, int numberOfSentences)
        {
            ...
            foreach (var term in wordFeatures)
            {
                ...
                double n00 = numberOfSentences - (n11 + n10 + n01);//Neither occured
        ...
        private static void AddTermFrequency(Dictionary<string, Dictionary<string, int>> termFrequencies, string token, int docName)
        {
